...
(function(){
  ...
  function _gmGet(k,d){ return (typeof GM_getValue!=='undefined') ? GM_getValue(k,d) : ''; }
  ...
  css.id = 'omega-theme';
  css.textContent = ''
    +'#antt-helper{position:fixed;top:20px;right:20px;z-index:999999;background:rgba(14,18,30,0.97);border:1px solid rgba(255,255,255,0.06);border-radius:16px;padding:16px;box-shadow:0 8px 40px rgba(0,0,0,0.5);font-family:"Segoe UI",Arial,sans-serif;width:440px;color:#c8cdd8;backdrop-filter:blur(20px);transition:all 0.3s ease}'
    +'.om-hidden{opacity:0!important;pointer-events:none!important;transform:scale(0.95)}'
    +'@media(max-width:768px){#antt-helper{position:fixed!important;bottom:0!important;left:0!important;right:auto!important;top:auto!important;width:100%!important;max-height:85vh;overflow-y:auto;border-radius:20px 20px 0 0;padding:12px 14px;box-shadow:0 -8px 40px rgba(0,0,0,0.6)}#antt-helper.om-hidden{transform:translateY(100%)!important;pointer-events:none;opacity:0}#omega-tabs{grid-template-columns:repeat(auto-fit,minmax(60px,1fr))!important}#omega-tabs button{padding:6px 4px!important;font-size:10px!important}.om-grid-2{grid-template-columns:1fr 1fr}.om-input{font-size:14px!important;padding:10px!important}.om-btn{padding:12px 16px!important;font-size:13px!important}.om-header{padding:4px 0 8px;margin-bottom:6px}}'
    +'#omega-fab{display:none;position:fixed;z-index:999998;width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#1a73e8,#1557b0);color:#fff;font-size:16px;font-weight:900;letter-spacing:1px;border:none;cursor:pointer;box-shadow:0 4px 20px rgba(26,115,232,0.4);align-items:center;justify-content:center;transition:transform 0.2s;user-select:none;touch-action:none}'
    +'#omega-fab:active{transform:scale(0.92)}#omega-fab.om-fab-connected{background:linear-gradient(135deg,#34a853,#2d8f47);box-shadow:0 4px 20px rgba(52,168,83,0.4)}'
    +'#omega-toasts{position:fixed;bottom:70px;left:50%;transform:translateX(-50%);z-index:999997;display:flex;flex-direction:column-reverse;gap:8px;pointer-events:none;max-width:90vw}'
    +'.om-toast{background:rgba(14,18,30,0.95);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:10px 16px;font-size:12px;color:#c8cdd8;backdrop-filter:blur(20px);box-shadow:0 4px 20px rgba(0,0,0,0.4);animation:toastIn 0.3s ease,toastOut 0.3s ease 2.7s forwards;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:340px;font-family:"Segoe UI",Arial,sans-serif}'
    +'.om-toast-ok{border-left:3px solid #34a853}.om-toast-err{border-left:3px solid #e07065}'
    +'@keyframes toastIn{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:none}}@keyframes toastOut{to{opacity:0;transform:translateY(-10px)}}'
    +'.om-swipe-handle{width:40px;height:4px;background:rgba(255,255,255,0.2);border-radius:2px;margin:0 auto 8px;display:none}@media(max-width:768px){.om-swipe-handle{display:block}}'
    +'.om-header{text-align:center;margin-bottom:10px;cursor:grab;user-select:none}.om-header:active{cursor:grabbing}.om-logo{font-size:22px;font-weight:800;color:#1a73e8;letter-spacing:4px}.om-sub{font-size:10px;color:#555e70;letter-spacing:2px;text-transform:uppercase}.om-close,.om-min{position:absolute;top:14px;cursor:pointer;font-size:15px;color:#555e70;transition:color 0.2s;user-select:none}.om-close:hover,.om-min:hover{color:#c8cdd8}.om-close{right:16px}.om-min{right:40px}'
    +'#omega-tabs{display:grid;gap:4px;margin-bottom:12px}#omega-tabs button{padding:8px;border:1px solid rgba(26,115,232,0.2);border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;letter-spacing:0.5px;transition:all 0.2s;background:rgba(26,115,232,0.06);color:#5a9cf5}#omega-tabs button:hover{background:rgba(26,115,232,0.12);border-color:rgba(26,115,232,0.3)}#omega-tabs button.om-aba-ativa{background:linear-gradient(135deg,#1a73e8,#1557b0);color:#fff;border-color:transparent;box-shadow:0 2px 12px rgba(26,115,232,0.3)}'
    +'.om-hr{border:none;border-top:1px solid rgba(255,255,255,0.05);margin:12px 0}.om-rodape{display:flex;align-items:center;gap:6px}.om-api-status{font-size:10px;color:#555e70;flex:1}.om-btn-api{padding:5px 10px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:6px;font-size:10px;color:#8a92a6;cursor:pointer;transition:all 0.2s}.om-btn-api:hover{background:rgba(255,255,255,0.08);color:#c8cdd8}'
    +'.om-btn{padding:9px 16px;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;letter-spacing:0.3px}.om-btn:active{transform:scale(0.97)}.om-btn-blue{background:linear-gradient(135deg,#1a73e8,#1557b0);color:#fff;box-shadow:0 2px 12px rgba(26,115,232,0.25)}.om-btn-green{background:linear-gradient(135deg,#34a853,#2d8f47);color:#fff;box-shadow:0 2px 12px rgba(52,168,83,0.25)}.om-btn-purple{background:linear-gradient(135deg,#6f42c1,#5a35a0);color:#fff;box-shadow:0 2px 12px rgba(111,66,193,0.25)}.om-btn-coral{background:linear-gradient(135deg,#e07065,#c0392b);color:#fff;box-shadow:0 2px 12px rgba(224,112,101,0.25)}.om-btn-amber{background:linear-gradient(135deg,#f0ad4e,#d4941f);color:#fff;box-shadow:0 2px 12px rgba(240,173,78,0.25)}.om-btn-sm{padding:5px 12px;font-size:11px;border-radius:6px}.om-btn-full{width:100%}.om-btn-list{padding:5px 10px;border:none;border-radius:6px;font-size:11px;cursor:pointer;transition:all 0.2s;font-weight:500}.om-btn-del{background:rgba(192,57,43,0.12);color:#e07065;border:1px solid rgba(192,57,43,0.15)}'
    +'.om-input{width:100%;padding:8px 10px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:8px;font-size:12px;color:#c8cdd8;outline:none;transition:border-color 0.2s;box-sizing:border-box}.om-input:focus{border-color:rgba(26,115,232,0.4);background:rgba(255,255,255,0.06)}.om-input::placeholder{color:#3a3f4e}.om-input-sm{padding:6px 8px;font-size:11px}.om-select{width:100%;padding:7px 10px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:8px;font-size:11px;color:#c8cdd8;outline:none;box-sizing:border-box;cursor:pointer}.om-select option{background:#0e121e;color:#c8cdd8}.om-label{font-size:10px;color:#555e70;letter-spacing:0.5px;display:block;margin-bottom:3px}.om-section-title{font-size:10px;font-weight:600;color:#8a92a6;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px}.om-box{margin-top:6px;font-size:11px;border-radius:8px;padding:8px 12px;line-height:1.5}.om-box-ok{background:rgba(52,168,83,0.08);color:#5ddb7a;border:1px solid rgba(52,168,83,0.15)}.om-box-err{background:rgba(192,57,43,0.08);color:#e07065;border:1px solid rgba(192,57,43,0.15)}.om-preview{font-size:10px;color:#555e70;min-height:14px;margin-top:3px}.om-preview .ok{color:#5ddb7a}.om-preview .warn{color:#f0ad4e}.om-badge{font-size:11px;font-weight:600;color:#fff;background:linear-gradient(135deg,#1a73e8,#1557b0);border-radius:6px;padding:3px 10px;display:inline-block;margin-bottom:8px}.om-grid{display:grid;gap:6px}.om-grid-2{grid-template-columns:1fr 1fr}.om-grid-3{grid-template-columns:1fr 1fr 1fr}.om-grid-21{grid-template-columns:2fr 1fr}.om-flex{display:flex;gap:6px}.om-mb-sm{margin-bottom:6px}.om-mb{margin-bottom:8px}.om-dropzone{border:2px dashed rgba(26,115,232,0.25);border-radius:10px;padding:16px;text-align:center;cursor:pointer;margin-bottom:8px;transition:all 0.2s}.om-dropzone:hover{border-color:rgba(26,115,232,0.45);background:rgba(26,115,232,0.04)}.om-dropzone-active{border-color:rgba(26,115,232,0.5);background:rgba(26,115,232,0.06)}.om-drop-txt{font-size:11px;color:#555e70}.om-drop-txt span{font-size:10px}.om-hist-item{display:flex;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px solid rgba(255,255,255,0.04)}.om-hist-item:last-child{border-bottom:none}.om-hist-placa{font-size:12px;font-weight:700;color:#c8cdd8}.om-hist-tempo{font-size:10px;color:#555e70}.om-hist-scroll{max-height:220px;overflow-y:auto}.om-vazio{font-size:11px;color:#3a3f4e;text-align:center;padding:20px 0}.om-resumo{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:10px;padding:10px 12px;margin-bottom:8px;font-size:11px;line-height:1.7}.om-resumo-label{color:#555e70;font-size:10px}.om-resumo-valor{color:#c8cdd8;font-weight:600}.om-resumo-aleatorio{color:#f0ad4e;font-style:italic}.om-log{background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.05);border-radius:8px;padding:6px 8px;max-height:140px;overflow-y:auto;font-family:monospace;font-size:10px;color:#8a92a6;line-height:1.6;margin-top:6px}.om-log-ok{color:#5ddb7a}.om-log-err{color:#e07065}.om-log-warn{color:#f0ad4e}';
  ...
  window.OmegaUtils = {
    ...
    addSecao: function(html){var c=document.getElementById('omega-content'); if(c) c.insertAdjacentHTML('beforeend',html);},
    getApiKey: function(){return _gmGet('omega_api_key','')||localStorage.getItem('omega_api_key')||'';},
    setApiKey: function(key){_gmSet('omega_api_key',key);localStorage.setItem('omega_api_key',key);},
    ...
    carregarHistorico: function(){try{var raw=_gmGet(this.HIST_KEY,'[]')||localStorage.getItem(this.HIST_KEY)||'[]';var self=this;return JSON.parse(raw).filter(function(i){return(Date.now()-i.ts)<self.HIST_TTL;});}catch(e){return[];}},
    salvarHistorico: function(lista){var raw=JSON.stringify(lista);_gmSet(this.HIST_KEY,raw);localStorage.setItem(this.HIST_KEY,raw);},
    ...
    toast: function(msg, ok){var t=document.createElement('div');t.className='om-toast '+(ok!==false?'om-toast-ok':'om-toast-err');t.textContent=msg;var c=document.getElementById('omega-toasts');if(c){c.appendChild(t);ST(function(){try{t.remove();}catch(e){}},3000);}while(c&&c.children.length>5) c.removeChild(c.firstChild);}
  };
  ...
  unsafeWindow.OmegaConfigAPI = function(){var atual=window.OmegaUtils.getApiKey();var nova=prompt('Cole sua chave API (sk-ant-...):',atual?'********':'');if(nova&&nova!=='********'){window.OmegaUtils.setApiKey(nova.trim());_atualizarStatusAPI();}};
