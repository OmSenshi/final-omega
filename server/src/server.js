...
app.post('/api/devices/stop', authGuard, (req, res) => {
  let stopped = 0;
  for (const [id, dev] of devices) {
    try { dev.ws.send(JSON.stringify({ type: 'stop' })); dev.status = 'idle'; dev.currentTask = null; stopped++; } catch(e) {}
  }
  res.json({ success: true, stopped });
});

// ═══ /api/govbr/poll — HTTP Polling pra dispositivos no Gov.br (CSP impede WebSocket) ═══
const govbrDevices = new Map();
const govbrTasks = new Map();

app.post('/api/govbr/poll', authGuard, (req, res) => {
  const { deviceId, name, status, message, detalhes } = req.body;
  if (!deviceId) return res.status(400).json({ error: 'deviceId obrigatorio' });

  // Registra/atualiza dispositivo Gov.br
  govbrDevices.set(deviceId, { name: name || deviceId, status: status || 'idle', lastPoll: Date.now() });

  // Também registra nos devices normais pra aparecer no painel
  if (!devices.has(deviceId)) {
    devices.set(deviceId, { ws: null, name: name || deviceId, status: status || 'idle', connectedAt: new Date().toISOString(), currentTask: null, _alive: true, _govbr: true });
  ...
  // Processa status vindo do dispositivo
  if (status && status !== 'idle') {
    broadcastToFrontend({ event: 'device_status', deviceId, status, message: message || '' });
    if (whatsappBot) {
      if (status === 'error' || status === 'error_critical') whatsappBot.sendError(message || 'Erro', '').catch(() => {});
      if (status === 'erro_fatal' && detalhes) whatsappBot.sendBloqueio(detalhes).catch(() => {});
      if (status === 'done') whatsappBot.sendToGroup('✅ ' + (message || 'Concluido!')).catch(() => {});
    }
  }

  // Verifica se tem tarefa pendente pra este dispositivo
  const pendingTask = govbrTasks.get(deviceId);
  if (pendingTask) {
    govbrTasks.delete(deviceId);
    return res.json({ type: 'task', task: pendingTask });
  }

  // Tenta pegar da fila geral se o dispositivo está livre
  if (status === 'idle' && taskQueue.length > 0) {
    const item = taskQueue.shift();
    return res.json({ type: 'task', task: item.task });
  }

  res.json({ type: 'none' });
});

// Limpa dispositivos Gov.br que pararam de fazer polling (>15s)
setInterval(() => {
  const now = Date.now();
  for (const [id, dev] of govbrDevices) {
    if (now - dev.lastPoll > 15000) {
      govbrDevices.delete(id);
      const d = devices.get(id);
      if (d && d._govbr) { devices.delete(id); broadcastToFrontend({ event: 'device_disconnected', deviceId: id }); }
    }
  }
}, 10000);
